...
LassiStand.prototype.addFlavor = function (flavor, price) {
  if (price <= 0) return -1;
  const flavourExits = this.menu.some((item) => item.flavor === flavor);
  if (flavourExits) return -1;
...
LassiStand.prototype.takeOrder = function (customerName, flavor, quantity) {
  ...
  const menuExits = this.menu.find((item) => item.flavor === flavor);
  if (!menuExits) return -1;
  ...
  const order = {
    ...
    flavor,
    quantity,
    total: menuExits.price * quantity,
    status: "pending",
  };
